...
| `EMAIL_TO` | `the job seeker's email` — comma-add yourself to shadow what he gets | yes |
| `RECIPIENT_NAME` | `the job seeker` | no (default the job seeker) |
...
| `CANDIDATE_PROFILE` | the FULL private profile markdown (the operator has the file: `PRIVATE-candidate-profile.md`) — overrides the committed, salary-scrubbed `profile/candidate.md` | strongly recommended (public repo) |
...
| `EDITION_URL` | `https://YOURNAME.github.io/YOUR-REPO` (after step 2) |
...
| `RAV_MONTHLY_TARGET` | monthly application quota from the RAV advisor (typically 10–12) — powers the RAV meter and the exported form's "N of T" line; unset = meter off |

## 2. GitHub Pages (the full edition / Archive / Grove)

Settings → **Pages** → Source: "Deploy from a branch" → Branch `main`,
folder `/docs` → Save. The site appears at
`https://YOURNAME.github.io/YOUR-REPO/` after the first evening
run commits `docs/`.

## 3. Fill in the private pieces

- Upload the four knowledge files + `voice-sample.md` to the Claude Project
  (see `claude-project/SETUP.md`) — use the PRIVATE filled versions the operator
...
## Daily rhythm (what the job seeker experiences)

- ~6:25 AM Zurich: silent scan banks the morning's postings
- ~3:00 PM: VM LinkedIn scan adds contacts (if the VM is set up)
- **6:30 PM Zurich**: ONE email from Vern — top jobs scored and
  explained, the RAV meter, "Start here" prompts, reach-out contacts,
  AI reading — linking to the full edition, Archive, and Grove on the
  web. (The workflow fires at 6:00 PM local year-round via a DST-aware
  ...
  the scheduled run late, which it routinely does.)
- ~2nd of the month: the filled RAV proof-of-efforts form arrives
  (see onboarding/11-rav-compliance.md)

## Troubleshooting

- **No email** — Actions tab → open the failed Evening Edition run.
  `SMTP_PASS` errors = app password wrong/revoked. "SMTP_USER/SMTP_PASS
  not set" = the secrets from onboarding step 3 were never added (the
  scans still run fine without them; only the send fails).
- **Evening Edition sits "pending" or gets cancelled** — something else
  is holding the `digest` concurrency group. Check the Actions tab for a
  run that is queued or still running; the edition waits behind it, and
  a newer pending run cancels an older one.

...
agent and a paid-news inbox feed. Cost: ~$6–9/month. Time: about an hour the
...
2. **Create → Droplet**. Choose:
   - Region: Frankfurt (closest to Zurich)
...
cd ~/Downloads && curl -O https://raw.githubusercontent.com/YOURNAME/YOUR-REPO/main/linkedin_auth.py
python3 linkedin_auth.py
```

A Chrome window opens on linkedin.com/login. Log in with **the job seeker's
...
Why the job seeker's account: the "Meet the hiring team" panel and warm-path
...
YOUR claude.ai account (the operator's Max), and paste the code it gives you
...
themes) and email/commit a one-pager so the job seeker wakes up prepared. A
starting cron (edit with `crontab -e`):

```cron
# 23:30 Zurich nightly (21:30 UTC = 23:30 CEST; use 22:30 UTC in winter)
30 21 * * * cd /home/vern/dossiers && ./run-dossiers.sh >> dossier.log 2>&1
```

where `run-dossiers.sh` clones/pulls the repo, reads `curated_data`'s
4–5★ jobs from the day's edition in docs/, and calls
`claude -p "Research <company> for a finance-manager candidate interview... write
...
## Part 7 (optional) — paid business-news digests in the radar

Subscription business news (e.g. Finanz und Wirtschaft, NZZ) is often
paywalled and protective of automated access. **Do not point a scraper at
a paywalled site.**

The safe route: most of them send subscribers a **daily headline email**.
If the job seeker subscribes to one worth having during the search:

1. In their Gmail, create a filter: from the publication's domain → apply
   a label (and optionally forward to a dedicated address).
2. Ask Claude (this project) to build the **inbox-watcher** upgrade on the
   VM: an IMAP script (Gmail app password for their account) that reads
   the labelled email each morning and folds executive moves, expansions,
   and funding items into the evening edition's regional radar — reading
   mail they legitimately receive, no site scraping, no ToS drama.

The same inbox-watcher, once built, also catches application replies the
tracker's Gmail scan patterns miss, and flags "reply needed" items into

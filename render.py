...
DEFAULT_EDITION_URL = ""  # set the EDITION_URL repo variable instead
...
<table role="presentation" width="620" cellpadding="0" cellspacing="0" border="0" style="max-width:620px;width:100%;background:#EFE7D6;border:1px solid #DCD0B4;">
  ...
  <tr><td style="background:#1E2E1A;padding:40px 34px 34px;text-align:center;">
    ...
    <div style="font-family:Helvetica,Arial,sans-serif;font-size:11px;letter-spacing:2.2px;text-transform:uppercase;color:#C6C3A6;padding-top:20px;">{stats.get("postings_read", 0)} postings read &middot; {stats.get("apply", 0)} apply-worthy &middot; {stats.get("home", 0)} in the {esc(cfg.HOME_REGION)}{" &middot; RAV: " + esc(stats["rav"]) if stats.get("rav") else ""}</div>
  </td></tr>

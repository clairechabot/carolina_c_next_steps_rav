...
the full edition carries NO "note from Vern" - the digest sits
...
TIER_LABELS = {k: v for k, v in cfg.TIER_LABELS.items() if k}

# Recruiter watch panel (links only) - lives in client/config.py.
FIRMS = cfg.RECRUITER_WATCH
...
/* the daily groaner (one fresh dad joke per edition; optional) */
    ...
    if (cfg.webhook) {
      try {
        fetch(cfg.webhook, { method: 'POST', mode: 'no-cors', body: JSON.stringify({
          ...
          address: b.dataset.address || '', workload: b.dataset.workload || '',
...
def job_row(job: dict, number: int, with_prompt: bool = True) -> str:
    ...
    save_btn = (f'<div class="actions"><button class="saveb" type="button" '
                ...
                f'data-address="{esc(job.get("address", ""))}" '
                f'data-workload="{esc(job.get("workload", ""))}" '
                f'data-method="{esc(job.get("method", ""))}">'
                f'&#9734; Save</button></div>')
    ...
    if with_prompt and job.get("kickoff_prompt"):
        ...
        body.append(f'''<details class="prompt">
            ...
            <div class="inner">
              ...
              <div class="addlist">
                <div class="k">What to add to the chat:</div>
                <ul>{items}</ul>
              </div>
            </div>
          </details>''')
...
def inplay_row(r: dict, number: int) -> str:
    ...
    missing = [label for label, key in (("employer address", "address"),
                                        ("workload %", "workload"),
                                        ("how applied", "method"))
               if r.get("status") in ("Applied", "Heard back", "Interview",
                                      "Offer") and not r.get(key)]
    if missing:
        body.append('<div class="watch"><div class="k">RAV fields missing'
                    f'</div><p>The tracker row has no {esc(", ".join(missing))}. '
                    'Fill it in the sheet so the RAV monthly tab is complete '
                    '- a one-line ask to Vern fetches an employer address.'
                    '</p></div>')
    rail = [f'<div class="no">No. {number:02d}</div>',
...
def build_edition(data: dict, now: datetime.datetime) -> str:
    ...
    sections = [
        ...
        ("recruiters", cfg.RECRUITER_WATCH_TITLE,
         ...
         [f'<article class="job"><div class="rail"><div class="no">{i + 1:02d}'
          ...
          f'<p class="summary">{esc(cfg.RECRUITER_WATCH_BLURB)}</p></div></article>'
...
<header class="cover">
  <div class="cover-in">
    <div class="eyebrow">Evening Edition &nbsp;·&nbsp; {esc(date_long)} &nbsp;·&nbsp; Zurich time</div>
    ...
    <div class="stats">
      ...
      <span>{stats.get("home", 0)} in the {esc(cfg.HOME_REGION)}</span>{f'<span class="dot"></span><span>RAV: {esc(stats["rav"])}</span>' if stats.get("rav") else ""}
    </div>
    {tracker_btn}
  </div>
</header>
